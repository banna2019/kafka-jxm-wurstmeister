...
sed -i 's#KAFKA_HEAP_OPTS="-Xmx1G -Xms1G"#KAFKA_HEAP_OPTS="-server -Xms2G -Xmx2G -XX:PermSize=128m -XX:+UseG1GC -XX:MaxGCPauseMillis=200 -XX:ParallelGCThreads=8 -XX:ConcGCThreads=5 -XX:InitiatingHeapOccupancyPercent=70"#g'  ${KAFKA_HOME}/bin/kafka-server-start.sh
sed -i '/KAFKA_HEAP_OPTS=/a\    export JMX_PORT="9099"' ${KAFKA_HOME}/bin/kafka-server-start.sh
